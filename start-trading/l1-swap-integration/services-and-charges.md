...
1. The current order book depth can absorb the swap (use [Integration APIs](api-market-depth.md "mention") to check whether the book depth would fill the order)
...
The charges are reflected in [Integration APIs](api-market-depth.md "mention"), so when you see the market depth from the API the fee is already taken into account.
