...
1. The current order book depth can absorb the swap (use [apis.md](apis.md "mention") to check whether the book depth would fill the order)
...
The charges are reflected in [apis.md](apis.md "mention"), so when you see the market depth from the API the fee is already taken into account.
