...
1. The current order book depth can absorb the swap (use [GET /swapIntent/depth](api-market-depth.md#get-swapintent-depth-symbol) to check whether the book depth would fill the order)
...
The charges are reflected in [Integration APIs - Market Depth](api-market-depth.md#get-swapintent-depth-symbol), so when you see the market depth from the API the fee is already taken into account.
