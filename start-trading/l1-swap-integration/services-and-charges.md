...
1. The current order book depth can absorb the swap (use [#get-swapintent-depth-symbol](api-market-depth.md#get-swapintent-depth-symbol "mention") to check whether the book depth would fill the order)
...
* For `ADAUSDCx`, in total 0.2% will be charged (0.1% for DeltaDeFi trading fee)
* For `NIGHTADA`, in total 0.4% will be charged (0.2% for DeltaDeFi trading fee)

The charges are reflected in [#get-swapintent-depth-symbol](api-market-depth.md#get-swapintent-depth-symbol "mention"), so when you see the market depth from the API the fee is already taken into account.
